...
namespace Writersword.Modules.TextEditor.Models.Document
{
    ...
    public sealed class ListProperties
    {
        ...
        /// <summary>
        /// Позиция маркера от левого поля (pt), которой раскладка реально пользуется при
        /// отрисовке. Совпадает с <see cref="MarkerIndentPt"/>, пока текст первой строки
        /// помещается справа от номера. У правого края текстовой зоны (и особенно в узкой
        /// ячейке) текст упирается в предел и дальше вправо не идёт — тогда номер сдвигается
        /// влево, чтобы не оказаться поверх текста. Транзиентное поле, не сериализуется.
        /// </summary>
        [JsonIgnore]
        public double ComputedMarkerIndentPt { get; set; }

...
class Program
{
    ...
    {
        // Консоль Windows работает в OEM-кодовой странице (866/437), где байт 0x07
        // отображается как «•». При выводе строки .NET подбирает для символа U+2022
        // ближайший байт этой кодовой страницы и подставляет 0x07, а консоль трактует
        // его как BEL и подаёт звуковой сигнал. Маркер списка «•» попадает в лог, и
        // каждая такая строка звучит. UTF-8 подстановку снимает.
        try
        {
        ...
        {
            // Запуск без присоединённой консоли: вывода нет, настраивать нечего.
        }

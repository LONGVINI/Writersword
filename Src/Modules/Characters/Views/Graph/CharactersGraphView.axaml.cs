...
namespace Writersword.Modules.Characters.Views.Graph
{
    /// <summary>
    /// Code-behind View графа связей персонажей.
    /// Рендеринг через SkiaSharp: Control + ICustomDrawOperation — как в DocumentCanvas.
    /// Поддерживает pan, zoom (колесо мыши), drag узлов,
    /// клик — открыть карточку, ПКМ — фокус на персонаже.
    /// </summary>
    public partial class CharactersGraphView : UserControl
    {
        ...
        private void OnDataContextChanged(object? sender, EventArgs e)
        {
            ...
            // GraphCanvas — x:Name из XAML, GraphRenderCanvas — внутренний Control рендеринга
        ...
        // ── Control для SkiaSharp рендеринга ─────────────────────────────

        /// <summary>
        /// Avalonia Control для рендеринга графа через ICustomDrawOperation.
        /// Переименован в GraphRenderCanvas чтобы не конфликтовать
        /// с x:Name="GraphCanvas" из XAML.
        /// </summary>
        private sealed class GraphRenderCanvas : Control
        {
            ...
            {
                // Читаем цвет фона из текущей темы Avalonia
                ...
                ctx.Custom(new GraphDrawOperation(
                    _viewModel,
                    bgColor,
            ...
            private sealed class GraphDrawOperation : ICustomDrawOperation
            {
                ...
                {
                    // Заливаем фон цветом из темы Avalonia
                ...
                private void DrawNodes(SKCanvas canvas)
                {
                    foreach (var node in _viewModel.Nodes)
                    {
                        ...
                        {
                            Color = SKColors.White.WithAlpha((byte)alpha),
                            IsAntialias = true
                        };
                        canvas.DrawText(
                            node.FallbackIcon,
                            cx, cy + iconSize * 0.35f,
                            SKTextAlign.Center,
                            iconFont, iconPaint);

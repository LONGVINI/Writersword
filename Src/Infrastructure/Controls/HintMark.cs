...
namespace Writersword.Infrastructure.Controls
{
    ...
    ///
    /// Шаблон и цвета заданы прямо здесь, а не файлом стилей. Контрол
    /// компилируется в две сборки — приложение и общие контролы, — а стиль
    /// из приложения не применился бы к типу из второй сборки: селектор
    /// сверяет тип вместе с его сборкой. В коде такой разницы нет, и значок

...
namespace Writersword.Core.Interfaces.Workspace
{
    ...
    public interface IWorkspaceController : IDisposable
    {
        ...
        /// После Suspend() выполняет мягкую реактивацию: layout и модули живы, пересоздания нет
        /// </summary>
        void Activate();

        /// <summary>
        /// Мягкая приостановка workspace при уходе с вкладки.
        /// Модули и layout остаются живыми — возврат на вкладку мгновенный,
        /// без пересоздания модулей и повторной загрузки данных.
        /// Останавливается автосейв, состояние сбрасывается в кеш.
        /// При наличии float-окон выполняется полная деактивация (запасной путь).
        /// </summary>
        void Suspend();

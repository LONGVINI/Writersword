...
namespace Writersword.Modules.TextEditor.Models.Settings
{
    ...
    public sealed class ReadingSettings
    {
        ...
        /// <summary>
        /// Гарнитура чтения. Пусто — как в документе.
        ///
        /// Здесь, а не в виде, и по той же причине, что и ступень размера: это
        /// поправка под конкретные глаза, а не оформление книги. В виде она делала бы
        /// смену цвета бумаги ещё и сменой начертания рукописи — молча и неожиданно.
        ///
        /// На саму рукопись не влияет никогда: подменяется только отрисовка чтения.
        /// </summary>
        public string? FontFamily { get; set; }

        // Пределы ступени. Они не про «правильный» размер букв: читателю никто не
        // указ, каким кеглем ему удобно. Это граница, за которой раскладка перестаёт
        // быть раскладкой — строка выше листа целиком, слово шире колонки, и число
        // страниц растёт быстрее, чем документ успевает пересобираться.
        //
        // 0,42× снизу и 4,3× сверху. Прежние −8…+20 при линейном шаге упирались в
        // потолок 2,2× — ступени за ним просто переставали работать, и человеку со
        // слабым зрением крупнее было уже не сделать. Дальше этих границ начинается
        // не чтение: строка выше листа целиком, а страниц столько, что документ не
        // успевает пересобираться.
        public const int MinFontStep = -15;
        public const int MaxFontStep = 25;

        // Во сколько раз меняет кегль одна ступень. Множитель, а не прибавка: шаг
        // берётся от ТЕКУЩЕГО размера, и нажатие ощущается одинаково на любом конце
        // шкалы. При линейной прибавке каждый шаг вниз отнимал долю исходного кегля,
        // и на мелком тексте шаги выходили всё грубее — под конец буквы уменьшались
        // сразу вдвое.
        private const double FontStepFactor = 1.06;
        ...
        public double FontScale
        {
            get
            {
                ...
                return Math.Pow(FontStepFactor, step);
            }
        }

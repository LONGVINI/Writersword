...
namespace Writersword.Modules.TextEditor.Models.Page
{
    ...
    public sealed class CanvasSettings
    {
        ...
        /// <summary>
        /// Свой цвет каретки (HEX). Пусто — каретка берёт цвет у текста, который пишет,
        /// и меняется вместе с ним: это верное поведение по умолчанию, потому что попасть
        /// в цвет текста нельзя ошибиться.
        ///
        /// Задаётся тем, кому такая каретка плохо видна: на тёмном листе она сливается со
        /// строкой, которую как раз и правят. Цвет здесь, рядом с цветом листа и текста, —
        /// он из того же разговора о том, как выглядит рабочая область, и на печать не
        /// влияет ровно так же.
        /// </summary>

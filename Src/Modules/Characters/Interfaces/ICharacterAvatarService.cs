...
namespace Writersword.Modules.Characters.Interfaces
{
    public interface ICharacterAvatarService
    {
        ...
        /// <summary>
        /// Порядок папок в списках — опознавателями, сверху вниз. Папки, которых
        /// в списке нет, встают после перечисленных в порядке по умолчанию:
        /// сначала папки проекта, затем общие, в конце встроенные.
        /// </summary>
        IReadOnlyList<string> GetPackOrder();

        /// <summary>
        /// Запомнить порядок папок. Список один на все области хранения: папку
        /// переносят между проектом и приложением свободно, и два раздельных
        /// списка разошлись бы при первом же переносе.
        ///
        /// Опознаватели, которых сейчас нет ни в одной области, из списка не
        /// выкидываются: локальные папки принадлежат открытому проекту, и при
        /// работе с другим проектом их порядок должен пережить сеанс.
        /// </summary>
        void SetPackOrder(IReadOnlyList<string> packIds);
        ...
        /// <summary>
        /// Вернуть запись в «Недавние» — отмена крестика по Ctrl+Z.
        ///
        /// Место задаётся соседом снизу, а не номером: пока окошко открыто,
        /// список может пересобраться (поставили аватарку — она уехала
        /// наверх), и номер после этого показывает уже на чужую запись.
        /// Соседа же ищут по адресу файла, и он остаётся собой.
        ///
        /// beforeRef пустой или не найден — запись встаёт в конец: значит,
        /// убирали последнюю, либо соседа с тех пор убрали и самого.
        /// </summary>
        void RestoreRecentAvatar(string? avatarRef, string? beforeRef);
